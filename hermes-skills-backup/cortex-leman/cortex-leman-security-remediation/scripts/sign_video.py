...
Signs videos with Content Provenance Authenticity metadata (C2PA / JUMBF).
Validated Aug 2026 on African Heroes Ep. 2 (Mami Wata).

Usage:
  python sign_video.py <input_video> <output_video> [--title "Title"] [--project PROJECT]
  python sign_video.py <video> /dev/null --verify-only

Setup (one-time):
  pip install c2pa-python
  git clone --depth 1 https://github.com/contentauth/c2pa-rs.git /tmp/c2pa-rs

Production: replace test certs with CA-issued X.509 cert (set C2PA_CERT_DIR env var).
"""
...
# Project-specific metadata presets
PROJECTS = {
    "african-heroes": {
...
def load_signer():
    """Load C2PA signer from certificate files (ES256 algorithm)."""
    ...
    if not os.path.exists(cert_path) or not os.path.exists(key_path):
        ...
        print(f"   Clone: git clone --://github.com/contentauth/c2pa-rs.git /tmp/c2pa-rs")
        sys.exit(1)

    # MUST read as bytes, not strings
    with open(cert_path, "rb") as f:
        cert_bytes = f.read()
    with open(key_path, "rb") as f:
        key_bytes = f.read()

    # C2paSignerInfo expects bytes for all fields
    signer_info = c2pa.C2paSignerInfo(
        alg=c2pa.C2paSigningAlg.ES256,
        sign_cert=cert_bytes,
        private_key=key_bytes,
        ta_url=None  # None for testing; use TSA URL in production
    )
    # MUST call create_signer_from_info — don't pass signer_info to sign_file
    return c2pa.c2pa.create_signer_from_info(signer_info)


def build_manifest(title, project_name):
    ...
    manifest = {
        ...
        "assertions": [
            ...
            {
                "label": "c2pa.actions",
                # First action MUST be "created" or "opened" (c2pa spec requirement)
                "data": {
                    "actions": [
                        {
                            "action": "created",
...
def sign_video(input_path, output_path, title, project_name):
    """Sign a video with C2PA metadata. Returns result dict."""
...
def verify_video(video_path):
    ...
    return {
        ...
        "validation_state": raw.get("validation_state", "Unknown")
    }
...
def main():
    ...
    if "untrusted" in str(info["validation_state"]).lower() or \
       "invalid" in str(info["validation_state"]).lower():
        print(f"\n⚠️  Test certificate — expected in dev. "
              f"Use production CA cert for official content.")
