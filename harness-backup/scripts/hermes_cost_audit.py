...
def get_session_detail(conn: sqlite3.Connection, session_id: str) -> dict:
    ...
    # Total session cost from breakdown
...
def audit_report(days: int = 7, source: str = None, budget: float = None) -> dict:
    ...
    # Unknown-cost sessions (pricing gap)
    ...
    # Cost by model (from session_model_usage)
...
def print_terminal(report: dict):
    ...
    print(f"\n🔓 PRICING COVERAGE")
    print(f"   Priced:     {pc['sessions_with_pricing']}/{pc['total_sessions'] if 'total_sessions' in pc else s['total_sessions']} ({pc['pct_sessions_priced']}%)")
