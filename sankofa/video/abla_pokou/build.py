#!/usr/bin/env python3
"""Build Abla Pokou SHORT (<60s): recut from original segments.

Keeps: 0(hook), 1(rivière), 2(identity), 7(sacrifice), 9(new CTA)
Cuts:  3(context), 4(marche), 5(marche), 6(comoé), 8(baouli)
"""
import json, os, subprocess, shutil
from pathlib import Path

BASE = Path("/home/tars/african-heroes/CHANNEL/video3_abla_pokou")
BROLL_IMG_DIR = BASE / "broll"
TMP_DIR = BASE / "tmp"
CLIPS_DIR = BASE / "clips"
TMP_SHORT_DIR = BASE / "tmp_short"
CAPTIONS_DIR = BASE / "captions"
AUDIO_SHORT_DIR = BASE / "audio_short"
TMP_SHORT_DIR.mkdir(parents=True, exist_ok=True)
...
# Short recut config: segments to keep with their audio files
SHORT_BEATS = [
    {"id": "01_hook",     "audio": "01_hook.mp3",     "image": "01_hook_river.png",     "direction": "zoom_in",  "caption_overlay": True},
    {"id": "02_riviere",  "audio": "02_riviere.mp3",  "image": "02_sacrifice_water.png", "direction": "zoom_out", "caption_overlay": False},
    {"id": "03_identity", "audio": "03_identity.mp3", "image": None,                     "direction": "zoom_in",  "caption_overlay": False},
    {"id": "07_sacrifice","audio": "07_sacrifice.mp3", "image": "02_sacrifice_water.png", "direction": "zoom_in",  "caption_overlay": False},
    {"id": "10_cta",      "audio": "cta_short.mp3",   "image": "09_baoule_heritage.png", "fullscreen": True,     "from_short_dir": True},
]
...
def build_kenburns(image_path, dur, out_path, direction="zoom_in"):
    ...
    tmp_scaled = TMP_SHORT_DIR / f"_kb_scaled_{image_path.stem}.png"
...
def main():
    ...
    for i, beat in enumerate(SHORT_BEATS):
        ...
        # Determine audio source and duration
        if beat.get("from_short_dir"):
            audio_path = AUDIO_SHORT_DIR / beat["audio"]
        else:
            audio_path = BASE / "audio" / beat["audio"]
        
        dur = get_dur(str(audio_path))
        audio_paths.append(audio_path)
        
        cap_png = CAPTIONS_DIR / f"cap_{bid}.png"
        seg = TMP_SHORT_DIR / f"seg_{i:02d}.mp4"
        ...
        if beat.get("fullscreen"):
            ...
            if beat["image"]:
                ...
                if ipath.exists():
                    kb_vid = TMP_SHORT_DIR / f"kb_{i:02d}.mp4"
                ...
                build_caption_segment(cap_png, dur, seg)
                print(f"    [seg {i}] {bid} 📝 caption only ({dur:.1f}s)")
        seg_paths.append(seg)

    # 2. Concat video
    print("\n[2/4] Concaténation vidéo...")
    concat_list = TMP_SHORT_DIR / "concat.txt"
    with open(concat_list, "w") as f:
        for p in seg_paths:
            f.write(f"file '{p.resolve()}'\n")
    video_concat = TMP_SHORT_DIR / "video_only.mp4"
    subprocess.run([
    ...
    audio_concat_list = TMP_SHORT_DIR / "audio_concat.txt"
    with open(audio_concat_list, "w") as f:
        for p in audio_paths:
            f.write(f"file '{p.resolve()}'\n")
    audio_full = TMP_SHORT_DIR / "audio_full.mp3"
    subprocess.run([
        "ffmpeg", "-y", "-f", "concat", "-safe", "0",
    ...
    # Merge video + audio
    video_audio = TMP_SHORT_DIR / "video_audio.mp4"
    subprocess.run([
    ...
    out_path = CLIPS_DIR / "abla_pokou_short.mp4"
